"""
Weather Engine — Open-Meteo live proxy with Kolar fallback.
Replaces legacy micro_climate_engine; deterministic spray-window logic.
"""
import logging
...
DEFAULT_LAT: float = settings.default_lat
DEFAULT_LON: float = settings.default_lon
DEFAULT_LOCATION: str = settings.default_location
...
def evaluate_spray_window(temperature: float, humidity: int, precip_prob: int, weather_code: int, condition: str) -> Tuple[bool, str]:
    if precip_prob >= 50:
...
def _fallback_weather(lat: float, lon: float, location_name: str, reason: str) -> WeatherData:
    ...
    logger.warning(f"Weather fallback engaged ({reason}) -> {safe_lat:.4f},{safe_lon:.4f}")
    temp = 28.5
    humidity = 62
    precip = 10
    code = 1
...
async def fetch_weather(latitude: Optional[float] = None, longitude: Optional[float] = None, location_name: Optional[str] = None) -> WeatherData:
    try:
    ...
    try:
        ...
        spray_safe, advisory = evaluate_spray_window(temperature, relative_humidity, precipitation_probability, w_code, condition)
